...
public class AutoPhrasingTokenFilter extends TokenFilter {
  ...
  public boolean incrementToken() throws IOException {
    if (!emitSingleTokens && unusedTokens.size() > 0) {
      //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO, "emitting unused phrases");
      ...
      if (emitSingleTokens && currentSetToCheck != null && currentSetToCheck.size() > 0) {
    	  ...
    	  if (lastEmitted != null && !equals( fixWhitespace(lastEmitted), getCurrentBuffer(new char[0] ))) {
            ...
            if (unusedTokens.size() > 0) {
              Token aToken = unusedTokens.remove( 0 );
              //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO,  "emitting putback token");
    	      emit( aToken );
              return true;
            }
    	  }
    ...
    if (currentSetToCheck == null || currentSetToCheck.size() == 0 ) {
      //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO, "Checking for phrase start on '" + new String( nextToken ) + "'" );
      ...
      //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO,  "returning at end." );
      return incrementToken( );
    }
  }
  ...
  private void emit( char[] token ) {
	//Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO,  "emit: " + new String( token ) );
  ...
  private CharArrayMap convertPhraseSet( CharArraySet phraseSet ) {
	...
	while (phraseIt != null && phraseIt.hasNext() ) {
	  ...
	  //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO, "'" + new String( phrase ) + "'" );
			
	  char[] firstTerm = getFirstTerm( phrase );
	  //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO, "'" + new String( firstTerm ) + "'" );
  ...
  private void discardCharTokens( StringBuffer phrase, ArrayList<Token> tokenList ) {
	//Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO, "discardCharTokens: '" + phrase.toString() + "'" );
	OffsetAttribute offAttr = getOffsetAttribute( );
	...
	for (int i = 0; i < phrase.length(); i++ ) {
	  ...
	  if (isSpaceChar( chAt ) && i > lastSp) {
		...
		if (lastEmitted == null || !endsWith( lastEmitted, tok )) {
		  ...
		  //Logger.getLogger(AutoPhrasingTokenFilter.class.getName()).log(Level.INFO, "discard " + new String( tok ) + ": " + token.startPos + ", " + token.endPos );
		  tokenList.add( token );
		}
		lastSp = i+1;
	  }
